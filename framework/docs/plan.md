...
| M16 | Harness-efficiency objective + evolvable context policy (SoL-Pi) | ✅ | **125 passed** (5 new in `test_harness_efficiency.py`). Fourth objective — `projects/harness_efficiency/` `EfficiencyObjective` — folds token spend into correctness (`correctness − λ·min(1, tokens/500)`, quality-first scalar floor): concise-correct accepted over verbose-correct (val 0.91→0.97), concise-wrong screened (~−0.01); the val threshold + M13 paired net-gain floor enforce SoL-Pi's predeclared capability floor [2609.20519] *unchanged*. New POLICY artifact `policy/context` bounds injected memory; `build_spec` truncates to `max_memory_chars`; the same floor accepts context compaction (policy v1→v2). `Attempt.input_chars` exposes input cost additively | `ArtifactType.POLICY` + `policy/bound-context` operator (catalog 8→9) + `policy` in `FAST_SURFACES`; golden event bytes unchanged |
| M17 | RSIAgent lessons: gap map + infra-unscored + scoped insights (CR-1/2/3) | ✅ | **138 passed** (12 new in `test_m17.py`). CR-1: improver sees a train-split gap map (`{gaps}` slot in `meta/improver-template`) — **train-only, val aggregate, test sealed** — derived from the memoized active train book (no extra eval; val/test task ids provably absent). CR-2: `TaskScore.infra` + infra-aware `ScoreBook.mean`; `code_tasks.evaluate` classifies sandbox timeout / spawn `OSError` / signal-kill as infra; `selection.paired_scores` aligns over commonly-scored tasks (`None` when none compare); the val gate fail-closes when inconclusive; EVAL events + persisted scorebooks carry `n_infra`/`infra`. CR-3: `Insight.scope` (round-trips, defaults "") + `[applies when: …]` render prefix; accept-distill records `{surface}/{operator}` scope | golden regenerated (9 EVAL lines gain `"n_infra":0`, verdicts unchanged); infra → "UNSCORED" in `render.scorebook` |
